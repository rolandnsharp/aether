...
const gain = amount => signal => t => signal(t) * amount;
...
wave('dub-bass', dubEffect(sin(110)));
wave('dub-lead', dubEffect(sin(880)));
wave('soft-pad', softEffect(sin(220)));

// Compose chains
const masterChain = pipe(dubEffect, gain(0.3));
wave('master', masterChain(sin(440)));
```
...
```javascript
wave('jam', pipe(sin(440), gain(2), clip(0.7), reverb(0.8), gain(0.3)));
```
...
```javascript
wave('fast', t => Math.sin(2 * Math.PI * 440 * t) * 0.3);
```
